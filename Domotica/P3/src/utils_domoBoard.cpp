...
void rele_pulsador(tsSensor *Sensor){
	static int valor = OFF;
	if(domoboard.RELE_INTERRUPTOR.sActuador.valor>0){
		valor = ON;
	}
	...
	if(Sensor->valor_Df == Sensor->valor){
		...
		if(valor == OFF){
			...
			domoboard.activarActuador(&domoboard.RELE_INTERRUPTOR, 1);
		}else{
			valor = OFF;
			Serial.println("RELE OFF");
			domoboard.activarActuador(&domoboard.RELE_INTERRUPTOR, 0);
		}
	}
}
