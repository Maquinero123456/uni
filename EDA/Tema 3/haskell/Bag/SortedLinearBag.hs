...
delete x (Node y oy s) | x==y && (oy-1)>0= Node y (oy-1) s
                       | x==y && (oy-1)==0= s
...
--------------------------------------------------------
-- selectores
isEmpty_empty = isEmpty empty == isEmpty Empty 
isEmpty_insert x s = isEmpty (insert x s) == False

occurrences_empty x = occurrences x empty == 0
occurrences_insert_1 x s = occurrences x (insert x s) > 0 
...
intersection (Node x ox s) (Node y oy t) | x==y = Node x (num) (intersection s t)
...
difference (Node x ox s) (Node y oy t) | x==y && ox<=oy = difference s t
