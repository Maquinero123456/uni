...
simplify (Node x y) (Unif z) = Node (Node x y) (Unif z)
simplify (Unif z) (Node x y)  = Node (Unif z) (Node x y)
simplify (Node x y) (Node z t) = Node (Node x y) (Node z t)
...
get x (V z t) = get' x z t

get' :: Eq a => Int -> Int -> Tree a -> a
get' _ _ (Unif a) = a
get' x y (Node t u) | x<div y 2 = get' x (div y 2) t
                    | otherwise = get' (x-div y 2) (div y 2) u

-------------------------------------------------------------------------------
-- | Exercise e.
set :: Eq a => Int -> a -> Vector a -> Vector a
set i x (V t u) = V t (set2 x i t u)

set2 :: Eq a => a -> Int -> Int -> Tree a -> Tree a
set2 x i sz (Unif t) | x == t = Unif t
                     | sz == 1 = Unif x
                     | i < div sz 2 = Node (set2 x i (div sz 2) (Unif t)) (Unif t)
                     | otherwise = Node (Unif t) (set2 x (i-div sz 2) (div sz 2) (Unif t))

set2 x i sz (Node t u) | i< div sz 2 = simplify (set2 x i (div sz 2) t) u
                       | otherwise = simplify t (set2 x (i- div sz 2) (div sz 2) u)
...
mapVector f (V t u) = V t (mapVector' f u)

mapVector' :: Eq b => (a -> b) -> Tree a -> Tree b
mapVector' f (Unif t) = Unif (f t)
