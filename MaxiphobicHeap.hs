...
merge h@(Node x sz lh rh) h'@(Node x' sz' lh' rh')
 | x < x'          = Node x (1+size lh + size rh + sz')  (ganador h' lh rh)  (perdedor h' lh rh)
 | otherwise       = Node x' (1+size lh + size rh + sz) (ganador h lh' rh')  (perdedor h lh' rh')
  where ganador h'@(Node x' sz' lh' rh') lh rh | sz' >= size lh && sz' >= size rh = h'
                                                | sz' < size lh && size lh >= size rh = lh
                                                | otherwise = rh
                                               
        perdedor h'@(Node x' sz' lh' rh') lh rh | sz' >= size lh && sz' >= size rh = merge lh rh
                                               | sz' < size lh && size lh >= size rh =  merge h' rh
                                               | otherwise = merge h' lh 
----------------------------------------------------------
-- ^^^^^^^^^^^^^^-- SOLO TOCAR ARRIBA ^^^^^^^^^^^ ---------
----------------------------------------------------------
